...
| B3 | Single run per arm; report Wilson CIs for accuracies and seeded bootstrap CIs for mean ρ; paired tests (Wilcoxon / McNemar) for within-model comparisons. | `summary.json` |
...
| C2 | Independent per-moment scoring beats joint for all 7 models (Δρ +.15…+.32, paired Wilcoxon p ≤ .03, CIs exclude 0): the binding failure is within-context relative calibration, not perception. | §3.5 G1 |
...
| C7 | Fine-tune result (pending): pre-registered success = Δρ_JOINT ≥ .10, paired p<.05; report null as null. | finetune/PLAN.md §5 |

## D. Limitations to write (not hide)

Creator concentration (A5); cultural strata tiny (n=22/8/120, descriptive only); single run
