...
| C15 | **All T2 fine-tune "gains" are format compliance, proven by control**: R2 (intensity-only, zero match examples) scores identically to match-trained R3/R4 on T2 (.402 vs .411/.402), fixes unparsed 17→0 identically, and on the base-parsed subset every arm's Δ is ≤+.011 (McNemar p=1.000). Cross-task transfer is nil both directions. Never quote R3's headline +.107/p=.036 without the parsed-subset null. | REPORT_R3.md, REPORT_R4.md §4 |
| C16 | **Multi-task supervision ties single-task, never beats it**: R4 vs R2 on T1 +.022/+.020 (p=.69/.64), R4 vs R3 on T2 −.009 (p=1.0). R4 clearing the prereg T1-JOINT bar (Δ+.134, p=.030) where R2 missed is a threshold artefact (R4−R2 itself p=.69) — do not write "multi-task crossed the bar". Collapse is intensity-supervision-specific: R3 (match-only) collapses 0/250, exactly like base. | REPORT_R4.md |
| C17 | **Audio shortcut demonstrated → within-video negatives are necessary, not conservative**: Gemini native video (audio) jumps +.232 on cross-video distractors (within .545 → XV .777, McNemar p=6e-6) while the same model on frames moves +.036 (n.s.); all 7 frame models n.s. on XV. Reaction clips carry stimulus-audio bleed, so cross-video distractors fall to soundtrack mismatch. The .777 is a shortcut score, never a capability number. | EVIDENCE §3.7 |
